...
The Lucky (`mavis`) executor is served by a same-device internal runtime
...
run code on the operator-approved machine. From Jormungand's perspective,
Lucky is dispatched through the same public `codex-bridge` endpoint as Codex;
the Codex bridge performs the internal same-device forwarding.
...
- Application adapter: `repos/jormungand/lib/agent-bridge.ts` chooses the
  Codex device bridge for Codex/Mavis, the OpenClaw device bridge, OpenClaw
  A2A, or simulation.
...
- Lucky same-device runtime: `repos/jormungand/scripts/lucky-mavis-server.mjs`
...
1. `lib/agent-bridge.ts` resolves both `codex` and `mavis` to
   `CODEX_BRIDGE_URL`.
2. `invokeConfiguredAgent` POSTs the bridge protocol payload to
   `${CODEX_BRIDGE_URL}/agent-runs`.
3. `codex-bridge.mjs` forwards the Mavis request to the same-device runtime
   at `LUCKY_BRIDGE_URL`.
4. The server (PID managed by `scripts/start-lucky-mavis-server.ps1`)
...
URL handled by `app/api/lucky/route.ts`) remains a client-side compatibility
surface. The dashboard's server-side dispatcher uses `CODEX_BRIDGE_URL` for
Mavis and does not create a separate application-facing MiniMax bridge device.
...
through `/health`; `/api/agent-health` remains behind site authentication and
reports the two device bridges: `codex-bridge` and `openclaw-bridge`. Mavis is
shown under the Codex device rather than as a third bridge.
