...
export async function getAgentQuotas(): Promise<AgentQuota[]> {
  ...
  const quotas = agentProfiles.map((profile, index): AgentQuota | null => {
    if (profile.id === "codex") return null
    const weeklyLimit = profile.family === "manual" ? 0 : 100000
    const weeklyUsed = profile.family === "manual"
      ? 0
      : [18000, 42000, 76000, 93000, 1000, 51000][index] ?? 0
    ...
    return {
      ...
      model: profile.family === "manual" ? "No model" : "Configured model",
      weeklyLimit,
      weeklyUsed,
      ...state,
      unit: "tokens",
  ...
  return [await getCodexQuota(), ...quotas]
}
